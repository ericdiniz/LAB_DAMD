...
class Database {
  constructor(){ this.dbPath = path.join(__dirname, 'tasks_grpc.db'); this.db = null; }
  async init(){ this.db = new sqlite3.Database(this.dbPath); await this.createTables(); console.log('✅ Database gRPC inicializado'); }
  async createTables(){
    ...
    const taskTable = `CREATE TABLE IF NOT EXISTS tasks (
      ...
      userId TEXT NOT NULL, createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  async getAllWithPagination(sql, params, page=1, limit=10){
    const offset=(page-1)*limit; const paginated=`${sql} LIMIT ? OFFSET ?`; const countSql=sql.replace(/SELECT.*FROM/,'SELECT COUNT(*) as count FROM');
    const [rows, count] = await Promise.all([ this.all(paginated,[...params,limit,offset]), this.get(countSql, params) ]);
    return { rows, total: count.count, page, limit, totalPages: Math.ceil(count.count/limit) };
  }
}
