...
/// Serviço utilitário para fluxo de câmera: lista câmeras, solicitar permissão,
/// abrir a tela de captura e salvar/excluir imagens no diretório do app.
class CameraService {
  CameraService._init();
  ...
  /// Retorna a câmera primária (primeira disponível) ou `null` se não houver.
  ...
  /// Abre a UI de captura e retorna o caminho salvo da imagem ou null.
  ...
  Future<String> savePicture(XFile image) async {
    ...
    final fileName =
        'task_${DateTime.now().millisecondsSinceEpoch}${path.extension(image.path)}';
